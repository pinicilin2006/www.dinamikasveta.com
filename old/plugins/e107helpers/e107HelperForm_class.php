 ...
 * <li>$Date: 2007/07/31 21:03:10 $</li>
 * </ul>
 * @author     $Author: Neil $
...
//         return;
    ...
    */
   function createForm($name, $type=HELPER_FORM_TYPE_E107_PREF, $action="", $method="post", $target="", $enctype="") {
      eval(HELPER_LOGGER_TRACE_METHOD_ENTRY);
      ...
      $this->_formTag = "<form $action$method$target$name$enctype>";
      $this->_formTag .= "<input type='hidden' name='".$this->_formName."' value='true'/>";
   ...
   function generateHTML($showPrompt=false, $showHelp=false, $message=false, $sql=false) {
      ...
      $keys = array_keys($this->_tags);
      $focus = ""; // Used to generate some javascript to set focus to first field on form
         ...
         if (($this->_batchMode == 1 || (!$tag->isBatchTag())) && $key != HELPER_ID_BATCH_GROUP) {
            if (is_a($tag, "e107HelperTagObj")) {
               if (strlen($tag->getLabel()) > 0) {
                  ...
                  $focus = $focus=="" ? $tag->getID("", "", false) : $focus;
                  if (!$this->_showErrorsAtTop() && $tag->isInError()) {
                     $text .= $this->getError($key, true);
                  }
               } else {
      ...
      $text .= $this->_tableStart.$this->_prettyPrint;
      $text .= $this->generateSubmitButtonHTML();
      $text .= "</table>".$this->_prettyPrint;
      $text .= "<input type='hidden' name='e107helper_selected_key' value='".$this->_getDBIndexValue()."'/>";
      $text .= $this->getHiddenTags();
      $text .= "</form>".$this->_prettyPrint;
      $text .= "<script type='text/javascript'>if (document.getElementById('$focus')) document.getElementById('$focus').focus();</script>".$this->_prettyPrint;
      $this->_formHTML = $text;
      eval(HELPER_LOGGER_TRACE_METHOD_EXIT);
   }
   ...
   function getErrorText() {
      ...
      foreach ($keys as $key) {
         ...
         if (is_a($this->_tags[$key], "e107HelperTagObj") && $this->_tags[$key]->isInError()) {
            $errtext .= "<span style='cursor:pointer;' onclick='document.getElementById(\"".$key."_0\").focus();'>";
