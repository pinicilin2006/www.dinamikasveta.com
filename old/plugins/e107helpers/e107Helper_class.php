 ...
 * <li>$Date: 2007/07/31 21:00:14 $</li>
 * </ul>
 * @author     $Author: Neil $
 * @version    $Revision: 1.9.2.16 $
...
class e107Helper {
   ...
   var $userclasslist;  // A comma separated list of calsses available to the current user
   /**#@-*/
    ...
    */
   function addUserClass($userid, $userclass) {
      global $sql;
      ...
      if (preg_match($userClassRegEx, $userclass) > 0) {
   	...
   	$row2 = $sql->db_Fetch();
   	if (!getperms("0") && !check_class($row2["userclass_editclass"])) {
   	   // Not allowed
   	   return;
   	}
   ...
   /**
    * Helper functions for formatting Ajax messages
    */
   function ajaxBegin() {
   ...
   function ajaxInnerHTML($id, $html) {
      return "<response type='innerhtml' id='$id'><![CDATA[$html]]></response>";
   }
