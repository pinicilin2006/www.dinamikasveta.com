 ...
 * <b>CVS Details:</b>
 * <ul>
 * <li>$Source: e:\_repository\e107_plugins/e107helpers/e107HelperCustomField_class.php,v $</li>
 * <li>$Date: 2007/07/31 21:02:03 $</li>
 * </ul>
 * @author     $Author: Neil $
 ...
 */
   class e107HelperCustomField {
      ...
      function getForm($id, $value, $customfields, $fieldname="") {
         ...
         $body .= "<tr><td>Name</td><td><input id='{$id}_name' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "name")."' type='text' class='tbox'$readonly/></td></tr>";
         $body .= "<tr><td>Label</td><td><input id='{$id}_label' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "label")."' type='text' class='tbox'/></td></tr>";
         $body .= "<tr><td>CSS Class for Field</td><td><input id='{$id}_field_class' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "field_class")."' type='text' class='tbox'/></td></tr>";
         $body .= "<tr><td>CSS Class for Label</td><td><input id='{$id}_label_class' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "label_class")."' type='text' class='tbox'/></td></tr>";
         switch ($value) {
            case "text" : {
               $header = " : Text";
               $body .= "<tr><td>Size           </td><td><input id='{$id}_size' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "size")."' type='text' class='tbox'/></td></tr>";
               $body .= "<tr><td>Maximum length </td><td><input id='{$id}_maxl' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "maxl")."' type='text' class='tbox'/></td></tr>";
               $checked = $customfields->getValue($fieldname, "mand")==1 ? " checked='true'" : "";
               $body .= "<tr><td>Mandatory </td><td><input id='{$id}_mand' name='{$id}_attrib' value='1' type='checkbox' class='tbox'$checked/></td></tr>";
               break;
            }
            case "textarea" : {
               $header = " : Text area";
               $body .= "<tr><td>Rows      </td><td><input id='{$id}_rows' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "rows")."' type='text' class='tbox'/></td></tr>";
               $body .= "<tr><td>Cols/Width</td><td><input id='{$id}_cols' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "cols")."' type='text' class='tbox'/></td></tr>";
               $checked = $customfields->getValue($fieldname, "mand")==1 ? " checked='true'" : "";
               $body .= "<tr><td>Mandatory </td><td><input id='{$id}_mand' name='{$id}_attrib' value='1' type='checkbox' class='tbox'$checked/></td></tr>";
               break;
            }
            case "select" : {
               $header = " : Select (list)";
               $body .= "<tr><td>1st row</td><td><input id='{$id}_row1' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "row1")."' type='text' class='tbox'/></td></tr>";
               $body .= "<tr><td>Options</td><td>";
               ...
               $body .= "</div></td></tr>";
               $body .= "<tr><td colspan='2' style='text-align:right;'><input type='button' onclick='javascript:duplicateHTML(\"selectline\",\"select_container\");' class='button' value='+'></td></tr>";
               break;
            }
            case "date" : {
               $header = " : Date";
               $body .= "<tr><td>Year from</td><td><input id='{$id}_yearfrom' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "yearfrom")."' type='text' class='tbox'/></td></tr>";
               $body .= "<tr><td>Year to  </td><td><input id='{$id}_yearto'   name='{$id}_attrib' value='".$customfields->getValue($fieldname, "yearto")."' type='text' class='tbox'/></td></tr>";
               $checked = $customfields->getValue($fieldname, "mand")==1 ? " checked='true'" : "";
               $body .= "<tr><td>Mandatory </td><td><input id='{$id}_mand' name='{$id}_attrib' value='1' type='checkbox' class='tbox'$checked/></td></tr>";
               break;
            }
            case "checkbox" : {
               $header = " : Checkbox";
               $body .= "<tr><td>Value</td><td><input id='{$id}_value' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "value")."' type='text' class='tbox'/></td></tr>";
               $body .= "<tr><td>Text</td><td><input id='{$id}_text' name='{$id}_attrib' value='".$customfields->getValue($fieldname, "text")."' type='text' class='tbox'/></td></tr>";
               break;
            }
            case "radio" : {
               $header = " : Radio buttons";
               $body .= "<tr><td>Text/Value</td><td>";
               $body .= "<div id='select_container' style='white-space:nowrap;'>";
               $count = 0;
               do {
                  $body .= "<span id='selectline' style='white-space:nowrap;'>";
      ...
      function getDialog($id, $body, $header="", $focus="", $cancel_event="", $cancel_text="Cancel", $ok_event="", $ok_text="Select") {
         ...
         // Help
         $text .= "<div class='forumheader3 smalltext' style='padding:2px;'>Some help text to go in here...</div>";
         // Buttons
